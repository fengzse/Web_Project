...
def upload():
    ...
    f.save(os.path.join(current_app.config['UPLOADED_PATH'], f.filename))  # 上传时返回找不到图片的url，应该是这里不对
...
def follow(username):
    ...
    db.session.commit()  # 这里没有db.session.add()而是直接commit提交数据库
...
def followed_by(username):  # 函数名称的意思是谁被这个用户关注了，也就是这个用户following了谁
    user = User.query.filter_by(username=username).first()
    if user is None:
        flash("Invalid user.")
        return redirect(url_for('.index'))
